...
actor MediaConverter {
    ...
    func convertImage(_ item: PhotosPickerItem?, format: ImageFormat) async -> String {
        guard let item, let data = try? await item.loadTransferable(type: Data.self), let image = UIImage(data: data) else { return "無法讀取圖片" }
        let output: Data?
        let ext: String
        switch format {
        case .jpeg: output = image.jpegData(compressionQuality: 0.95); ext = "jpg"
        case .png: output = image.pngData(); ext = "png"
        }
    ...
    func extractCover(_ item: PhotosPickerItem?) async -> String {
        guard let url = await temporaryURL(from: item, extension: "mov") else { return "無法讀取影片" }
        let asset = AVURLAsset(url: url)
        let generator = AVAssetImageGenerator(asset: asset)
        generator.appliesPreferredTrackTransform = true
        do {
            let cg = try generator.copyCGImage(at: .zero, actualTime: nil)
            guard let data = UIImage(cgImage: cg).jpegData(compressionQuality: 0.95) else { return "封面轉換失敗" }
            return await saveDataToPhotos(data, extension: "jpg") ? "封面已儲存" : "儲存失敗"
        } catch { return "擷取封面失敗：\(error.localizedDescription)" }
    }
    ...
    func videoToGIF(_ item: PhotosPickerItem?, quality: Double) async -> String {
        ...
        guard duration > 0 else { return "影片長度無效" }
        ...
        let out = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString).appendingPathExtension("gif")
        guard let dest = CGImageDestinationCreateWithURL(out as CFURL, UTType.gif.identifier as CFString, frameCount, nil) else { return "GIF 建立失敗" }
        ...
        guard CGImageDestinationFinalize(dest), let data = try? Data(contentsOf: out) else { return "GIF 輸出失敗" }
    ...
    func makeLivePhoto(photo: PhotosPickerItem?, video: PhotosPickerItem?) async -> String {
        guard let photoData = try? await photo?.loadTransferable(type: Data.self), let videoURL = await temporaryURL(from: video, extension: "mov") else { return "請選擇圖片與影片" }
        ...
        } catch { return "Live Photo 生成失敗：\(error.localizedDescription)" }
    }
    ...
    func exportLivePhoto(_ item: PhotosPickerItem?, as kind: LiveExport) async -> String {
        ...
        case .video:
            guard let url = await temporaryURL(from: item, extension: "mov") else { return "無法取得 Live Photo 影片" }
...
enum LivePhotoMetadata {
    static func writePhoto(data: Data, assetIdentifier: String, to output: URL) throws {
        guard let source = CGImageSourceCreateWithData(data as CFData, nil), let type = CGImageSourceGetType(source), let dest = CGImageDestinationCreateWithURL(output as CFURL, type, 1, nil) else { throw NSError(domain: "LivePhoto", code: 1) }
        ...
        guard CGImageDestinationFinalize(dest) else { throw NSError(domain: "LivePhoto", code: 2) }
    }
    ...
    static func writeVideo(source: URL, assetIdentifier: String, to output: URL) async throws {
        ...
        for track in try await asset.load(.tracks) {
            guard let dst = composition.addMutableTrack(withMediaType: track.mediaType, preferredTrackID: kCMPersistentTrackID_Invalid) else { continue }
        ...
        if exporter.status != .completed { throw exporter.error ?? NSError(domain: "LivePhoto", code: 3) }
    }
}
